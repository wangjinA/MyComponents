 ...
 * @LastEditors: 汪锦
 * @LastEditTime: 2020-06-23 17:31:14
  ...
  <div class="hyForm" :class="{'hyForm-readonly': readonlyType}">
    <Row :gutter="gutter">
      <i-col :span="item.row ? 24 : 24 / colNum" v-for="(item, index) in columns" :key="index">
        <div class="hyForm-item" :style="`margin-bottom: ${colGutter}px; ${readonlyType && 'align-items: flex-start;'}`">
          ...
          <div class="hyForm-item-content">
            ...
            <div v-else>
              <template v-if="item.type == 'select'">
                <i-select size="default" v-model="formData[item.key]" clearable @on-clear="selectClear(item)">
                  <i-option
                    :value="select.value"
              ...
              <template v-else>
                <i-input size="default" v-model="formData[item.key]" :placeholder="'请输入' + item.name" />
              </template>
            </div>
          </div>
        </div>
    ...
    <div class="hyForm-operation">
      <div v-show="searchType">
        <slot name="footer">
          <hy-button class="hy-button" ref="addBtn" @click="$emit('add')" v-if="$listeners.add" size="samll">添加</hy-button>
